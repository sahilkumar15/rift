...
# Status: MODIFIED
"""Run necessity/sufficiency audit for detector x explainer."""
...
def audit_one(
    ...
    *,
    ...
    with torch.no_grad():
        ...
        gn = adapter.identity_gap(
            nec,
            donor=donor,
            source_id=source_id,
            target_id=target_id,
        )
        gs = adapter.identity_gap(
            suf,
            donor=donor,
            source_id=source_id,
            target_id=target_id,
...
def aggregate(rows: List[Dict]) -> Dict:
    ...
    agg = {
        k: sum(r[k] for r in rows if r.get(k) is not None) / max(1, len(rows))
        for k in keys
    }

    agg["explainer"] = rows[0].get("explainer")
    agg["n"] = len(rows)
